...
const breakpoints = [75000, 150000, 300000, 600000, 1000000];
  ...
  useEffect(() => {
    ...
    case cap <= breakpoints[1]:
      barRef.current.style.setProperty(
        '--pseudo-before-width',
        `${
          ((cap - breakpoints[0]) * 25) / (breakpoints[1] - breakpoints[0]) +
            17
        }%`
      );
      break;
    case cap <= breakpoints[2]:
      barRef.current.style.setProperty(
        '--pseudo-before-width',
        `${
          ((cap - breakpoints[1]) * 25) / (breakpoints[2] - breakpoints[1]) +
            42
        }%`
      );
      break;
    case cap <= breakpoints[3]:
      barRef.current.style.setProperty(
        '--pseudo-before-width',
        `${
          ((cap - breakpoints[2]) * 25) / (breakpoints[3] - breakpoints[2]) +
            67
        }%`
      );
      break;
  ...
  return (
    <div className={styles.capBarRoot}>
      ...
      <div className={styles.capBar} ref={barRef}>
        ...
        <div>
          <span>Seed Cap</span> ($ {breakpoints[3].toLocaleString()})
        </div>
      </div>
    </div>
  );
